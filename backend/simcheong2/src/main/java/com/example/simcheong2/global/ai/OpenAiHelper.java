...
public class OpenAiHelper {
    ...
    public String getTextFromAudioMultipartFile(MultipartFile multipartFile, String uploadDirRealPath) {
        String uploadFileName = fileHelper.saveFile(multipartFile, uploadDirRealPath);
        if (uploadFileName == null) {
            throw new CustomException(ErrorCode.INTERNAL_SERVER_ERROR, "이미지 파일을 현재 처리할 수 없습니다. 잠시후 다시 시도해주세요.");
        }
        ...
        try {
            String responseText = analyzeAudioWithChatClient(uploadDirRealPath, uploadFileName);
            log.info(responseText);
            return responseText;
        } catch (Exception e) {
            log.error(e.getMessage());
    ...
    public String analyzeAudioWithChatClient(String uploadDirRealPath, String uploadFileName) {
        String audioFullPath = uploadDirRealPath + File.separator + uploadFileName;
        log.info("오디오 분석 시작! 경로: {}", audioFullPath);

        OpenAiService service = new OpenAiService(apiKey);

        CreateTranscriptionRequest request = new CreateTranscriptionRequest();
        request.setModel("whisper-1");
        File file = new File(audioFullPath);
        String transcription = service.createTranscription(request, audioFullPath).getText();
